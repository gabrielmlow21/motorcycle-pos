...
> ⚠️ **MODIFIED FOR THIS PROJECT.** The upstream version of this example returned a `float`
> from `get()` (`$value / 100`) and multiplied a float in `set()`. That violates
> `CLAUDE.md:37` — money is integer sen, never float — so it has been replaced below.
> Do not restore the upstream form. See `.claude/skills/payments-guardian/references/money-and-tax.md`.
...
// Money stays an integer number of sen end to end. Formatting for display
// happens in the API resource or the React component, never in the cast.
class SenCast implements CastsAttributes
{
    ...
    public function set($model, string $key, $value, array $attributes): int
    {
        if (! is_int($value)) {
            throw new InvalidArgumentException(
                "Money must be an integer number of sen, got: " . get_debug_type($value)
            );
        }
